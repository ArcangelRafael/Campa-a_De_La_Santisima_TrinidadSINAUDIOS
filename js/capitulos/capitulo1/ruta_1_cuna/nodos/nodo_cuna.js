    ...
    return new Promise(resolve => {
        ...
        if (!document.getElementById(styleId)) {
            ...
            style.innerHTML = `
                ...
                /* =========================================
                   HALOS INMORTALES DE LOS CAÍDOS
                   (Sobreviven al repintado del motor base)
                   ========================================= */
                .modo-combate .marcador-batalla.skull-icon,
                .modo-combate .skull-icon {
                    color: #ffffff !important;
                    text-shadow: 0 0 15px #ff0000, 0 0 30px #ff0000, 0 0 45px #ff0000 !important;
                    opacity: 0.95 !important;
                }
                .modo-combate .marcador-batalla.cross-icon,
                .modo-combate .cross-icon {
                    color: #ffd700 !important;
                    text-shadow: 0 0 15px #ffaa00, 0 0 30px #ffaa00, 0 0 45px #ffaa00 !important;
                    opacity: 0.95 !important;
                }

                /* TIERRA DE NADIE: Empuja la cruz a la derecha (hacia el enemigo) 
                   sin meterla en la tarjeta del enemigo para no borrar su imagen */
                .modo-combate #zona-aliada .marcador-batalla.cross-icon {
...
window.actualizarAvanceReservas = function() {
    ...
    let colIniciales = { 0: CONSTANTES_TACTICAS.COLUMNA_INICIO_TRASERAS, 1: CONSTANTES_TACTICAS.COLUMNA_INICIO_MEDIAS, 2: CONSTANTES_TACTICAS.COLUMNA_INICIO_PUNTA, 3: CONSTANTES_TACTICAS.COLUMNA_INICIO_MEDIAS, 4: CONSTANTES_TACTICAS.COLUMNA_INICIO_TRASERAS };

    let avances = EstadoBatalla.tropasVivas.map(pos => { return pos.col - colIniciales[pos.row]; });
    ...
    let traseraArriba = EstadoBatalla.tropasVivas.find(p => p.row === 0);
    let traseraAbajo = EstadoBatalla.tropasVivas.find(p => p.row === 4);

    let traseraArribaAvanzo = traseraArriba && traseraArriba.idUnico && jugador.tropas.find(t => t.idUnico === traseraArriba.idUnico)?.hp > 0 && !traseraArriba.ignorarMuerto && (traseraArriba.col > colIniciales[0]);
    let traseraAbajoAvanzo = traseraAbajo && traseraAbajo.idUnico && jugador.tropas.find(t => t.idUnico === traseraAbajo.idUnico)?.hp > 0 && !traseraAbajo.ignorarMuerto && (traseraAbajo.col > colIniciales[4]);

    let traserosVanguardiaAvanzan = (traseraArribaAvanzo || traseraAbajoAvanzo);

    let cabezasReserva = resZona.querySelectorAll("div:nth-child(5n + 1)");
    cabezasReserva.forEach(cabeza => {
        if (traserosVanguardiaAvanzan) { cabeza.style.setProperty("transform", "translateX(0px)", "important"); } 
        else { cabeza.style.setProperty("transform", "translateX(-40px)", "important"); }
    });
};

async function animarDialogoAvanceCuna() {
    let divSeparador = document.createElement("div"); divSeparador.innerHTML = "<br><br>"; storyArea.appendChild(divSeparador);
...
function animarAvanceCuna(esVictoriaFinal = false) {
    ...
    requestAnimationFrame(() => {
        ...
        if (typeof gestionarEfectosVisuales === "function") gestionarEfectosVisuales(tablero, "cuna");
            ...
            if(pos.idUnico) {
                ...
                if(tropa && tropa.hp > 0 && !pos.ignorarMuerto) {
                    ...
                    if (nextCol >= 0 && nextCol <= 2) {
                        ...
                        if (enemigo && !enemigo.muerto) { accion.tipo = "choque"; accion.enemigo = enemigo; }
        ...
        domUpdates.forEach(update => {
            ...
            if (knightEl) {
                ...
                if(targetSlot) {
                    targetSlot.appendChild(knightEl); knightEl.style.display = "block";
                    if (update.accion.tipo === "choque") {
                        knightEl.classList.add("caballero-atacando"); targetSlot.classList.add("en-combate"); 
                    } else if (update.accion.tipo === "ruptura" || update.accion.tipo === "avance_libre") {
                        let arrastrarReservas = false;
                        if (esVictoriaFinal && (update.row === 0 || update.row === 4)) { arrastrarReservas = true; } 
                        ...
                        setTimeout(() => { if (typeof animarRupturaLinea === "function") animarRupturaLinea(update.idUnico, arrastrarReservas); }, 50);
                    } else {
        ...
        if (!esVictoriaFinal) {
            ...
            setTimeout(() => { document.getElementById("formacion-overlay").style.display = "none"; if (typeof evaluarContinuacionBatalla === "function") evaluarContinuacionBatalla(); }, 4500);
        }
    });
}
...
function resolverDadosVisualesCuna() {
    ...
    let infoFe = obtenerEstadoFe(); let huboCombates = false; let autoCombat = document.getElementById("ht-auto-combat")?.checked;
    ...
    EstadoBatalla.accionesPendientes.forEach(acc => {
        ...
        if (tipo === "choque") {
            ...
            let pAtk = poderAtk.neto + dadoGracia + infoFe.mod; let pDef = enemigo.def + dadoFuria;
            ...
            let idBc = 'bc_' + Math.random().toString(36).substr(2,9); let idBcMelee = idBc + '_melee';
            let phase1Cons = ""; let hasMelee = false;

            if (pAtk > pDef) {
                enemigo.muerto = true; EstadoBatalla.bajasEnemigas++;
                phase1Cons = `<span class="mensaje-sistema">¡IMPACTO LETAL! La carga abate al infiel.</span>`; wantsToAdvance[pos.row] = true;
                
                window.marcadoresBatalla.push({tipo: 'skull', row: pos.row, col: nextCol});
                if(typeof window.dibujarMarcadorMuerte === 'function') window.dibujarMarcadorMuerte(targetSlot, 'skull');
            } else {
            ...
            if (hasMelee) {
                ...
                let pAtk2 = poderAtk.neto + dadoGracia2 + infoFe.mod; let pAtkEnemigo = enemigo.atk + dadoFuria2;
                let phase2Cons = "";

                if (pAtk2 > pAtkEnemigo) {
                    enemigo.muerto = true; EstadoBatalla.bajasEnemigas++;
                    phase2Cons = `<span class="mensaje-sistema">¡${tropa.nombre} se impone en la refriega!</span>`; wantsToAdvance[pos.row] = true;
    ...
    if (huboCombates || Object.values(wantsToAdvance).some(v => v)) {
        document.querySelectorAll('#zona-reservas .tropa-draggable').forEach(r => { r.classList.add('caballero-atacando'); setTimeout(() => r.classList.remove('caballero-atacando'), 400); });
    }

    let isAlive = (r) => { let p = EstadoBatalla.tropasVivas.find(x => x.row === r); return p && p.idUnico && jugador.tropas.find(t => t.idUnico === p.idUnico)?.hp > 0; };
    [0, 1, 2, 3, 4].forEach(r => { let p = EstadoBatalla.tropasVivas.find(x => x.row === r); if (isAlive(r) && wantsToAdvance[r]) p.col++; });

    if (typeof actualizarAvanceReservas === "function") actualizarAvanceReservas();
...
async function resolverFormacionCuna(resultado) {
    ...
    if (resultado.total === 5) {
        ...
        await MotorDialogos.mostrarDialogo({ personajeImg: "assets/img/personajes/aliados/jugador.webp", nombrePersonaje: `Comendador ${jugador.nombre}`, alineacion: "izq", bordeClase: "borde-comandante", nombreClase: "nombre-comandante", texto: `"¡A LA CARGA hijos del cielo! ¡Romped sus líneas!"`, claseTexto: "txt-comandante" });
        await MotorDialogos.mostrarDialogo({ personajeImg: "assets/img/personajes/aliados/lider_caballeromontado.webp", nombrePersonaje: lider, alineacion: "izq", bordeClase: "borde-aliado", nombreClase: "nombre-izq", texto: `"¡Que la sangre de mártires pinte nuestro camino y la ira del Altísimo guíe mi lanza! ¡Por la Cruz Bicolor, no dejaremos uno vivo! ¡DEUS LO VULT!"`, claseTexto: "txt-lugarteniente" });
        
        if (resultado.puntaEsNoble) { agregarTexto("¡Una formación táctica perfecta! La cuña está completamente cerrada con escudos al frente liderada por sangre noble. El impacto inicial contra los paganos es demoledor."); GestorEstado.modificarFe(30, "una formación perfecta e inspiradora"); } 
        else { await MotorDialogos.mostrarDialogo({ personajeImg: "assets/img/personajes/aliados/multicab.webp", nombrePersonaje: "Hueste de Caballeros", alineacion: "izq", bordeClase: "borde-aliado", nombreClase: "nombre-izq", texto: `"Una carga digna, pero la presencia de un veterano noble al frente habría quebrado sus espíritus antes del choque..."`, claseTexto: "txt-multitud" }); GestorEstado.modificarFe(25, "una formación completa sin noble al frente"); }
        GestorEstado.modificarAtaque(2, "una clara ventaja táctica");
    } else {
        let penalizacionFe = -(resultado.vacios * multiplicadorPenalizacion);
        await MotorDialogos.mostrarDialogo({ personajeImg: "assets/img/personajes/aliados/jugador.webp", nombrePersonaje: `Comendador ${jugador.nombre}`, alineacion: "izq", bordeClase: "borde-comandante", nombreClase: "nombre-comandante", texto: `"¡A LA CARGAAAAAAAA!"`, claseTexto: "txt-comandante" });
        
        if (tipoError === "sin_punta") { await MotorDialogos.mostrarDialogo({ personajeImg: "assets/img/personajes/aliados/multicab.webp", nombrePersonaje: "Hueste de Caballeros", alineacion: "izq", bordeClase: "borde-aliado", nombreClase: "nombre-izq", texto: `"¡Dime que compraste tu título noble sin decirme que compraste tu título noble! ¡Vamos a morir por esta mala formación sin líder al frente!"`, claseTexto: "txt-multitud" }); } 
        else if (tipoError === "mala_distribucion") {
            if (resultado.nombreLider) { await MotorDialogos.mostrarDialogo({ personajeImg: "assets/img/personajes/aliados/lider_caballeromontado.webp", nombrePersonaje: resultado.nombreLider, alineacion: "izq", bordeClase: "borde-aliado", nombreClase: "nombre-izq", texto: `"No logramos completar la formación pero ${jugador.nombre} ha demostrado saber liderar... Mi Señor, si esta aquí morimos, recíbenos en tu reino con los brazos abiertos y perdona nuestros pecados."`, claseTexto: "txt-lugarteniente" }); }
            await MotorDialogos.mostrarDialogo({ personajeImg: "assets/img/personajes/aliados/multicab.webp", nombrePersonaje: "Hueste de Caballeros", alineacion: "izq", bordeClase: "borde-aliado", nombreClase: "nombre-izq", texto: `"¡Si sobrevivo a esta formación tan horrible, desertaré de esta cruzada que fracasará!"`, claseTexto: "txt-multitud" });
        } else {
            if (resultado.total > 0 && resultado.nombreLider) { await MotorDialogos.mostrarDialogo({ personajeImg: "assets/img/personajes/aliados/lider_caballeromontado.webp", nombrePersonaje: resultado.nombreLider, alineacion: "izq", bordeClase: "borde-aliado", nombreClase: "nombre-izq", texto: `"No logramos completar la formación pero ${jugador.nombre} ha demostrado saber liderar... Mi Señor, si esta aquí morimos, recíbenos en tu reino con los brazos abiertos y perdona nuestros pecados."`, claseTexto: "txt-lugarteniente" }); }
            await MotorDialogos.mostrarDialogo({ personajeImg: "assets/img/personajes/aliados/multicab.webp", nombrePersonaje: "Hueste de Caballeros", alineacion: "izq", bordeClase: "borde-aliado", nombreClase: "nombre-izq", texto: `"¡Esta cruzada será un fracaso! ¡No es posible que ante el primer choque ni siquiera se haya completado una maldita formación en CUÑA!"`, claseTexto: "txt-multitud" });
        }
    ...
    crearBoton("⚔️ ¡QUE DIOS RECONOZCA A LOS SUYOS! (Iniciar Choque)", async () => {
        ...
        if(!skipCine && typeof playCinematicaCargaCuna === 'function') { await new Promise(res => playCinematicaCargaCuna(resultado, res)); }
        if(overlay) overlay.style.display = "none";
        let {victoria, bajas} = await iniciarCombateCuna(resultado);
        if(typeof evaluarFinCombateCuna === 'function') await evaluarFinCombateCuna(victoria, bajas);
    });
}